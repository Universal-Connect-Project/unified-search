...
async function loadData(){
  ...
  let versions = await axios.get(`${config.DATABASE_URL}version.json`).then(res => res.data);
  ...
  const response = await axios.get(config.DATABASE_URL + `db/${version}.csv`, {
    responseType: 'stream'
  }).catch(err => {
  ...
  const indexResponse = await axios.get(config.DATABASE_URL + `db/${version}.txt`, {
    responseType: 'stream'
  }).catch(err => {})
